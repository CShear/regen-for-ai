  ...
  private async acquireLock(): Promise<void> {
    ...
    while (true) {
      ...
      if (Date.now() - startedAt >= this.lockWaitMs) {
        throw new Error(
          `Timed out acquiring reconciliation run history lock after ${this.lockWaitMs}ms`
        );
      }
